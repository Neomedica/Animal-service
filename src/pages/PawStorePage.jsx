...
export default function PawStorePage() {
  ...
  function totalPrice() {
    return cart.reduce((s,i) => s + (Number(i.price.replace(/[^0-9.]/g,"")) * i.qty), 0);
  }
  ...
  function orderViaLine() {
    const lines = cart.map(i => i.emoji + " " + i.name + " x" + i.qty).join(", ");
    const msg = encodeURIComponent("สั่งซื้อ One of Our Paw by Furever: " + lines + " รวม " + totalPrice() + " บาท");
    const lineUrl = settings.line_url || "https://line.me";
    window.open(lineUrl + "?text=" + msg, "_blank");
  }

  const normal = products.filter(p => !p.is_quirky);
  const quirky = products.filter(p => p.is_quirky);
  const btnY = {background:"#FFD94D",color:"#3A2800",border:"none",borderRadius:"50px",padding:"10px 20px",fontSize:"13px",fontWeight:"800",cursor:"pointer",fontFamily:"var(--font-body)"};
  const grid = {display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:"16px",marginTop:"20px"};

  function ProductCard({p, quirkyStyle}) {
    const qty = getQty(p.id);
    return (
      <div style={{background:"white",borderRadius:"16px",border:"1.5px solid rgba(184,228,249,0.5)",overflow:"hidden"}}>
        <div style={{height:"120px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"52px",background:quirkyStyle?"linear-gradient(135deg,#F0E8FF,#FFFDE8)":"linear-gradient(135deg,#EEF8FF,#FFFDE8)"}}>
          {p.image_url?<img src={p.image_url} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<span>{p.emoji}</span>}
        </div>
        <div style={{padding:"12px 14px"}}>
          {p.tag&&<span style={{display:"inline-block",background:quirkyStyle?"#F0E8FF":"#EEF8FF",color:quirkyStyle?"#5B3FA0":"#185FA5",fontSize:"9px",fontWeight:"800",padding:"2px 8px",borderRadius:"50px",marginBottom:"6px"}}>{p.tag}</span>}
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            <button onClick={()=>addToCart(p)} style={{background:qty>0?"#0F6E56":"var(--blue-deep)",color:"white",border:"none",borderRadius:"50px",padding:"5px 12px",fontSize:"12px",fontWeight:"700",cursor:"pointer",fontFamily:"var(--font-body)"}}>
              {qty>0 ? ("✓ " + qty + " ชิ้น") : "+ ใส่ตะกร้า"}
            </button>
          </div>
        </div>
      </div>
    ...
    <main style={{padding:"0 0 80px"}}>
      <div style={{background:"linear-gradient(135deg,#1E2A3B,#2D4A6B)",padding:"60px 80px",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"48px"}}>
        <div>
          ...
          <button style={{...btnY,padding:"13px 28px",fontSize:"14px"}} onClick={()=>setShowCart(true)}>
      ...
      <div className="container">
        {normal.length>0&&(
          <div style={{marginBottom:"48px"}}>
            ...
            <div style={grid}>{normal.map(p=><ProductCard key={p.id} p={p} quirkyStyle={false}/>)}</div>
          </div>
        )}
        {quirky.length>0&&(
          <div style={{marginBottom:"48px"}}>
            ...
            <div style={grid}>{quirky.map(p=><ProductCard key={p.id} p={p} quirkyStyle={true}/>)}</div>
          </div>
        )}
                ...
                {cart.map(item=>(
                  <div key={item.id} style={{display:"flex",alignItems:"center",gap:"12px",padding:"12px 0",borderBottom:"1px solid #F0F4F8"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                      <button onClick={()=>updateQty(item.id,item.qty-1)} style={{width:"28px",height:"28px",borderRadius:"50%",border:"1px solid #D5E3EE",background:"white",cursor:"pointer",fontSize:"16px"}}>-</button>
